...
/**
 * fatal    logger.severe("Schwerwiegender Fehler"); error    logger.severe("Schwerwiegender Fehler"); warn
 * logger.warning("Warnung"); info     logger.info("Information"); config   logger.config("Konfigurationshinweis");
 * debug    logger.fine("Fein"); verbose  logger.finer("Feiner"); trace    logger.finest("Am feinsten");
 */
public class JavaLogLogger extends LoggerBase<Logger> {
    ...
    public void param(String sMethod, @NonNull String sParamName, Object oParamValue) {
        this.getLogger().config(this.buildLogMessage(sMethod, METHOD_INSIDE, METHOD_PARAM),
    ...
    public void warn(String sMethod, String sMessage, Object... oaParams) {
        StringUtils.join() this.getLogger().warning(this.buildLogMessage(sMethod, METHOD_INSIDE, sMessage), oaParams);
    }
