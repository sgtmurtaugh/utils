...
public interface BigIntegerMapperServletRequest extends ServletRequest {
    ...
    default BigInteger getBigIntegerParameter(String name) {
        return TypeMapperUtils.getDefaults().getBigIntegerMapper().map((String) this.getAttribute( name ));
    }

    default BigInteger getBigIntegerParameter(String name, BigInteger defaultValue) {
        return TypeMapperUtils.getDefaults().getBigIntegerMapper().map((String) this.getAttribute( name ), defaultValue);
    }

    default BigInteger getBigIntegerParameter(String name, boolean bTrim, boolean bEmptyIsNull) {
        return TypeMapperUtils.getDefaults().getBigIntegerMapper().map((String) this.getAttribute( name ), bTrim, bEmptyIsNull);
    }

    default BigInteger getBigIntegerParameter(String name, boolean bTrim, boolean bEmptyIsNull, BigInteger defaultValue) {
        return TypeMapperUtils.getDefaults().getBigIntegerMapper().map((String) this.getAttribute( name ), bTrim, bEmptyIsNull, defaultValue);
    }
