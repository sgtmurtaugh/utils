...
public interface DateScopeMapper extends ScopeMapper {
    ...
    default Date getDateAttribute(@NonNull String name) {
        return TypeMapperFactory.getDateMapper().mapObject(this.getAttribute(name));
    }
    ...
    default Date getDateAttribute(@NonNull String name, Date defaultValue) {
        return TypeMapperFactory.getDateMapper().mapObject(this.getAttribute(name), defaultValue);
    }
    ...
    default boolean hasDateAttribute(@NonNull String name) {
        if (StringUtils.isNotEmpty(name) && null != this.getAttribute(name)) {
            var value = TypeMapperFactory.getDateMapper().mapObject(this.getAttribute(name), null);
            return (null != value);
        }
        return false;
    }
    ...
    default boolean hasDateAttributeWithValue(@NonNull String name, Date value) {
        if (StringUtils.isNotEmpty(name) && null != this.getAttribute(name)) {
            return new EqualsBuilder().append(value,
                    TypeMapperFactory.getDateMapper().mapObject(this.getAttribute(name), null)).isEquals();
        }
        return false;
    }
