...
public interface DoubleScopeMapper extends ScopeMapper {
     ...
     * @return
     */
    default boolean hasDoubleAttribute(String name) {
        if (StringUtils.isNotEmpty(name) && null != this.getAttribute(name)) {
            var value = TypeMapperUtils.getDefaults().getDoubleMapper().mapObject(this.getAttribute(name), null);
            return (null != value);
        }
        return false;
    }
     ...
     * @return
     */
    default boolean hasDoubleAttributeWithValue(String name, Double value) {
        if (StringUtils.isNotEmpty(name) && null != this.getAttribute(name)) {
